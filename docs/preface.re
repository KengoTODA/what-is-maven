...
この資料は@<href>{http://maven.apache.org/,Maven}でJavaプロジェクトを開発する技術者に向けて整理されたものです。
...
@<b>{ビルド・ライフサイクル} （作業の順番）や、「依存元の前に依存先をビルド」といった @<b>{依存関係} をきちんと考えてビルドしてくれます。
...
#@warn(replace book.version.maven at preprocessing phase)

本資料は2019年11月時点での最新バージョン、@<code>{book.version.maven}を前提として情報をまとめています。このため上記の問題を解決し、読者に新鮮な
...
@<tt>{OSX 10.13.2}にて@<code>{Oracle JDK book.version.jdk}ならびに@<code>{Maven book.version.maven}を使用して動作確認を行っています。
...
本資料ならびに本資料が参照するソースコードは@<href>{https://github.com/KengoTODA/what-is-maven,GitHubで入手できます}。
...
本資料の文章部分は@<href>{https://creativecommons.org/licenses/by-nc/4.0/,Attribution-NonCommercial 4.0 International}によってライセンスされています.
またソースコードは@<href>{http://www.apache.org/licenses/LICENSE-2.0, "Apache License, Version 2.0"}によってライセンスされています。
...
文章ならびにソースコードの著作権は、特別の記載がない限り@<href>{mailto:skypencil@gmail.com,Kengo TODA}に帰属します。
...
本資料を作成するきっかけをくださった@<href>{https://twitter.com/ikeike443,@ikeike443}氏に御礼を申し上げます。
...
@<href>{https://github.com/KengoTODA/what-is-maven/releases,GitHubのリリースページ}から入手できます。@<href>{https://ipafont.ipa.go.jp/ipa_font_license_v1-html#jp,IPAフォントライセンスv1.0}に従い、@<href>{https://ipafont.ipa.go.jp/node17#jp,IPAexフォント}をエンベッドし利用しています。


=== ePUB, mobi


どちらも@<href>{https://github.com/KengoTODA/what-is-maven/releases,GitHubのリリースページ}から入手できます。


=== Kindle


@<href>{https://www.amazon.co.jp/dp/B01LXCT0TV,Amazon.co.jp}から入手できます。



注意点として、通常Amazonからは改訂版の更新情報を通達しません。つまり配布物が更新されてもお手元の端末の書籍が自動的に更新されることはありません。@<href>{https://kdp.amazon.co.jp/ja_JP/help/topic/G200966010,こちらのページ}を参考に、改訂版を入手してください。
