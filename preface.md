...
本資料は2015年2月時点での最新バージョン、`{{book.version.maven}}`を前提として情報をまとめています。このため上記の問題を解決し、読者に新鮮な
...
`OSX 10.10.2`にて`Oracle JDK{{book.version.jdk}}`ならびに`Maven {{book.version.maven}}`を使用して動作確認を行っています。
